...
source_in_list = ['honeyscrisp/NC1_t1', 'maize/Oh43x_SC', 'pear']
source_out_list = ['apple', 'maize', 'pear']

SOURCE_in = source_in_list[1]     # Replace
SOURCE_out = source_out_list[1]   # Replace   

TARGET_BANDS = 204           # if using full bands
RGB_BAND_INDICES = (70, 53, 19)

INPUT_DIR = f"{PAR_DIR}/1.raw/{SOURCE_in}/REFLECTANCE"
BASE_OUTPUT_DIR = f"{PAR_DIR}/2.pre_final/{SOURCE_out}/{TARGET_BANDS}_hsi_WR_mask"
rgb_dir = f"{PAR_DIR}/2.pre_final/{SOURCE_out}/pseudo_rgb_WR_mask"
...
def process_batch():
    ...
    # Compute WR mean spectrum (once); 1D spectral curve (per‐band reflectance curve); 각 밴드마다 하나씩 있는 값 (204 bands면 길이 204)
    # mean over pixels → spectrum over λ; x, y 방향으로 평균 but λ 방향은 평균하지 않음
