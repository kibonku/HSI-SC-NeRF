...
def generate_hsi_point_cloud(
    ...
    """Generate a hyperspectral point cloud.
    ...
    bands_list: List[Tensor] = []
    normals_list: List[Tensor] = []
    ...
    with progress as progress_bar:
        ...
        while total < num_points:
            ...
            hsi = outputs[hsi_output_name]          # [R, C]
            depth = outputs[depth_output_name]      # [R] or [R,1]
            if depth.ndim > 1:
                depth = depth.squeeze(-1)

            # opacity / accumulation mask
            if "accumulation" in outputs:
                opacity = outputs["accumulation"]
                if opacity.ndim > 1:
            ...
            rb = ray_bundle
            origins = rb.origins[mask]
            directions = rb.directions[mask]

            pts = origins + directions * depth.unsqueeze(-1)   # [M, 3]
            ...
            if crop_obb is not None:
                ...
                hsi = hsi[in_box]
                if normal is not None:
                    normal = normal[in_box]

            if pts.shape[0] == 0:
                progress_bar.advance(task, 0)
                continue

            points_list.append(pts.cpu())
            bands_list.append(hsi.cpu())
            if normal is not None:
                normals_list.append(normal.cpu())
    ...
    points = torch.cat(points_list, dim=0)[:num_points]
    bands = torch.cat(bands_list, dim=0)[:num_points]

    normals: Optional[Tensor] = None
    if normal_output_name is not None and len(normals_list) > 0:
        normals = torch.cat(normals_list, dim=0)[:num_points]

    return points, bands, normals
